...
def a_star(graph, start, cost, goal, explored = None ):
   global pq, path, pqh, hdict
   ...
   if(a_star(graph, ll[0], ll[1] - hdict[ll[0]], goal, explored)):
      return True
   return False
...
def prune_worse_paths(pq, index = 0):
   #get letter
   gl = lambda x:x[0]
   for i in reversed(range(index,len(pq))):
      if (gl(pq[index]) == gl(pq[i])):
         pq.remove(i)
         prune_worse_paths(pq, index)
         return
